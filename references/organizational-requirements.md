...
Workshop comparison baseline for the Copilot cross-check. Feed this file plus
the Figma screen to GitHub Copilot and ask for a coverage table.

> **Training status:** the rows below are a working baseline, not Phoenix's
> approved standard. Treat anything marked `[TO VERIFY]` as an assumption and
> do not convert it into an approved requirement. In real work, replace it with
> approved organizational policy or record it as an open question.

## 1. Content and messaging

| ID | Requirement | Source |
|---|---|---|
| CNT-01 | Every page states the product name and the insuring entity. | `[TO VERIFY]` |
| CNT-02 | A displayed premium is accompanied by a disclaimer that the final price depends on underwriting. | `[TO VERIFY — Regulation]` |
| CNT-03 | The primary call to action appears above the fold and is repeated once below. | Baseline |
| CNT-04 | No superlative claims such as "cheapest" or "fastest" without a dated source. | `[TO VERIFY]` |

## 2. Forms and data collection

| ID | Requirement | Source |
|---|---|---|
| FRM-01 | Collect only fields required for the stated purpose; each additional field has a documented justification. | `[TO VERIFY — Privacy]` |
| FRM-02 | Sensitive identifiers (ID number, license plate) are masked on input and never echoed back in plain text. | `[TO VERIFY]` |
| FRM-03 | Every required field has a visible required indicator **and** a defined validation message. | Baseline |
| FRM-04 | Marketing consent is opt-in, unchecked by default, with explicit wording naming the channels. | `[TO VERIFY — Marketing communications]` |
| FRM-05 | A link to the privacy notice appears adjacent to the form, not only in the footer. | `[TO VERIFY]` |
...
| LNG-02 | Hebrew is the primary language; mixed Latin terms appear only where no accepted Hebrew term exists. | `[TO VERIFY]` |
...
| ID | Requirement | Source |
|---|---|---|
| BRD-01 | Colors, typography and spacing come from the approved design system. | `[TO VERIFY]` |
| BRD-02 | Logo placement, clear space and minimum size follow brand guidelines. | `[TO VERIFY]` |
| BRD-03 | Button hierarchy is consistent: one primary action per view. | Baseline |
| BRD-04 | The hero uses orange #F58220 as its background and primary accent; green does not meet the workshop brand baseline. | Workshop demo baseline |
...
- Follow-up worth showing: ask it to turn the Missing rows into draft
  ...
  agent from Lab 5.
- BRD-04 is deliberately concrete for the workshop: the initial Figma prompt
  requests green, so participants can detect and correct a visible mismatch.
